...
NID_compare = function(tree_1, tree_2, cut_index_max = NULL){
  ...
  unlist(lapply(1:cut_index_max, FUN = function(cut_index) NID(cutree(tree_1, k = cut_index), 
                                                               cutree(tree_2, k = cut_index))))
}
ARI_compare = function(tree_1, tree_2, cut_index_max = NULL){
  if(is.null(cut_index_max)) cut_index_max = length(tree_1$order)
  unlist(lapply(1:cut_index_max, FUN = function(cut_index) ARI(cutree(tree_1, k = cut_index), 
                                                               cutree(tree_2, k = cut_index))))
}
...
corrplot2 = function (corr, method = c("circle", "square", "ellipse", "number", 
                      ...
                      addCoef.col = NULL, coef_thresh = 0.65,
                      ...
                      win.asp = 1, ...) 
{
  ...
  if(!is.null(order)){
    ...
    ord = order.dendrogram(as.dendrogram(order))
    corr <- corr[ord, ord]
  }
  ...
  if (method == "number" && plotCI == "n") {
    text(Pos2[, 1], Pos2[, 2], font = number.font, col = col.fill, 
  ...
  if (!is.null(addCoef.col) && method != "number") {
    text(Pos2[, 1], Pos2[, 2], col = addCoef.col, labels = round((DAT2 - 
  ...
  if (!is.null(addrect)) {
    corrRect.hclust2(corr, k = addrect, tree = order, 
                     col = rect.col, lwd = rect.lwd)
  }
  if(is.null(addrect) && !is.null(order)){
    corrRect.hclust3(corr, tree = order, 
                     col = rect.col, lwd = rect.lwd)
  }
  invisible(corr)
}
...
##  corrplot2 internal function to change the black rectangle display

corrRect.hclust2 = function (corr, k = 2, col = "black", lwd = 2, tree) 
{
  n <- nrow(corr)
  # method <- match.arg(method)
  tree <- tree
...
{
  n <- nrow(corr)
  # tree = hc_samples_dend
  tree <- tree
  hc = get_leaves_branches_col(tree)
